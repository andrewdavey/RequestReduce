...
.LocalNavigation .TabOn,.LocalNavigation .TabOn:hover {{
    ...
    background-position: {0};
        ...
        {
            [Theory]
            [InlineData("h1 {{float:left;}}", Spriting.PropertyCompletion.HasNothing)]
            [InlineData(@"h1 {{background-image: url(""http://i3.social.microsoft.com/contentservice/technet.png"");}}", Spriting.PropertyCompletion.HasImage)]
            [InlineData("h1 {{background-repeat: none;;}}", Spriting.PropertyCompletion.HasRepeat)]
            [InlineData("h1 {{background-position: 0;}}", Spriting.PropertyCompletion.HasXOffset)]
            [InlineData("h1 {{background-position: top;}}", Spriting.PropertyCompletion.HasYOffset)]
            [InlineData("h1 {{width:5px;}}", Spriting.PropertyCompletion.HasWidth)]
            [InlineData("h1 {{height:5px;}}", Spriting.PropertyCompletion.HasHeight)]
            [InlineData("h1 {{padding-left: 5px;}}", Spriting.PropertyCompletion.HasPaddingLeft)]
            [InlineData("h1 {{padding-right: 5px;}}", Spriting.PropertyCompletion.HasPaddingRight)]
            [InlineData("h1 {{padding-top: 5px;}}", Spriting.PropertyCompletion.HasPaddingTop)]
            [InlineData("h1 {{padding-bottom: 5px;}}", Spriting.PropertyCompletion.HasPaddingBottom)]
            public void WillIndicateIfAPropertyIsFilledIn(string css, Spriting.PropertyCompletion expectedCompletion)
            {
                var testable = new BackgroundImageClass(css, 0);
