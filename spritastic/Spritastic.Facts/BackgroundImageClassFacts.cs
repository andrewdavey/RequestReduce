...
.LocalNavigation .TabOn,.LocalNavigation .TabOn:hover {{
    ...
    background-position: {0};
        ...
        {
            [Theory]
            [InlineData("h1 {{float:left;}}", Spritastic.Parser.PropertyCompletion.HasNothing)]
            [InlineData(@"h1 {{background-image: url(""http://i3.social.microsoft.com/contentservice/technet.png"");}}", Spritastic.Parser.PropertyCompletion.HasImage)]
            [InlineData("h1 {{background-repeat: none;;}}", Spritastic.Parser.PropertyCompletion.HasRepeat)]
            [InlineData("h1 {{background-position: 0;}}", Spritastic.Parser.PropertyCompletion.HasXOffset)]
            [InlineData("h1 {{background-position: top;}}", Spritastic.Parser.PropertyCompletion.HasYOffset)]
            [InlineData("h1 {{width:5px;}}", Spritastic.Parser.PropertyCompletion.HasWidth)]
            [InlineData("h1 {{height:5px;}}", Spritastic.Parser.PropertyCompletion.HasHeight)]
            [InlineData("h1 {{padding-left: 5px;}}", Spritastic.Parser.PropertyCompletion.HasPaddingLeft)]
            [InlineData("h1 {{padding-right: 5px;}}", Spritastic.Parser.PropertyCompletion.HasPaddingRight)]
            [InlineData("h1 {{padding-top: 5px;}}", Spritastic.Parser.PropertyCompletion.HasPaddingTop)]
            [InlineData("h1 {{padding-bottom: 5px;}}", Spritastic.Parser.PropertyCompletion.HasPaddingBottom)]
            public void WillIndicateIfAPropertyIsFilledIn(string css, Spritastic.Parser.PropertyCompletion expectedCompletion)
            {
                var testable = new BackgroundImageClass(css, 0);
